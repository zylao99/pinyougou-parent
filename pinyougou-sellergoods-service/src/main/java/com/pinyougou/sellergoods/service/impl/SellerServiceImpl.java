...
public class SellerServiceImpl implements SellerService {
	...
	public void updateStatus(String sellerId, String status) {
		//update tb-seller set status=1 where seller_id = 1
		TbSeller seller = sellerMapper.selectByPrimaryKey(sellerId);//对象的状态 是0
		//要改成对应的
	 ...
	 */
	@Override
	public Map getSaleroom(String sellerId, Integer month) {
		...
		for (int i = month; i > 0; i--) {
			...
			Double total =orderMapper.getSaleroom1(map);
			...
			map1.put("name",format.format(date));
			seriesData.add(map1);
		}
